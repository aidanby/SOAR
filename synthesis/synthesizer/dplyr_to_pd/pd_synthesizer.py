        ...
        for i in range(n_mappings):
            ...
            while enumerator.has_next():
                ...
                if result.is_correct():
                    ...
                    ranks[src_api.id] = (enumerator.current_tree.primary_api.id, rank)
...
def load_dplyr(path):
    f = open('code_analysis/grammar.lark')
...
def load_csv(path):
    inputs = {}
    ro.globalenv['input1'] = ro.DataFrame.from_csvfile(path)
...
def run_benchmark(source_path, input_path):
    ...
    inputs = load_csv(f'./dplyr/inputs/{input_path}')
