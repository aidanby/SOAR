...
class TorchSynthesizer(Synthesizer):
    ...
    def __init__(self, source_program: Program, source_library: str,
        ...
        self.api_matcher = ApiMatching.get_matcher(source_library, target_library, use_embedding=True)
        self.input = input
...
def run_benchmark(benchmark, test_cases):
    s_src_lines, fp_src_lines = load_example_by_name(f'../benchmarks/{benchmark}')
    s_program = Program(s_src_lines)
    synth = TorchSynthesizer(s_program, 'tf', 'torch', input=test_cases[benchmark], err_enabled=True)
    start = time.time()
...
if __name__ == '__main__':
    ...
    results = []
    test_cases = {'discriminator': np.random.rand(100, 50, 40, 1), 'basic': np.random.rand(100, 50),
                  'conv': np.random.rand(100, 50, 40, 1), 'conv_m': np.random.rand(100, 50, 40, 1),
                  'nlp_conv1d': np.random.randint(50, size=(32, 10)),
                  'word2vec': np.random.randint(1000, size=(32, 10)),  'nlp': np.random.randint(50, size=(32, 10)),
                  'densenet': np.random.rand(10, 224, 224, 3),
                  'auto_encoder': np.random.rand(10, 7, 7, 32),
                  'alexnet': np.random.rand(10, 227, 227, 3),
                  'generator': np.random.rand(10, 6272),
                  'lenet': np.random.rand(10, 32, 32, 1),
                  'vgg11': np.random.rand(10, 224, 224, 3), 'vgg16': np.random.rand(10, 224, 224, 3),
                  'vgg19': np.random.rand(10, 224, 224, 3),
                  'densenet_part1': np.random.rand(10, 224, 224, 3),
                  'densenet_part2': np.random.rand(10, 224, 224, 3), 'lstm': np.random.rand(10, 28, 28, 1),
                  'densenet_transition_block': np.random.rand(10, 224, 224, 3)
                  }

    res = run_benchmark(cmd_args.benchmark, test_cases)
