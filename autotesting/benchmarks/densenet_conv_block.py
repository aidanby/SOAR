...
class TFConv(Model):
    def __init__(self, **kwargs):
        ...
        self.bn_1 = tf.keras.layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5)
        self.activation_1 = tf.keras.layers.Activation('relu')
        self.conv_1 = tf.keras.layers.Conv2D(4 * growth_rate, 1, use_bias=False)
        self.bn_2 = tf.keras.layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5)
        self.activation_2 = tf.keras.layers.Activation('relu')
        self.conv_2 = tf.keras.layers.Conv2D(growth_rate, 3, padding='same', use_bias=False)
